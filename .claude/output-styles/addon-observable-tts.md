...
description: Tool calls + git diffs + audio summary via Elevenlabs TTS
---
...
## Variables
- **USER_NAME**: `[Your name here]` *(Configure this with your name for personalized audio summaries)*
...
```typescript
Read({ file_path: "Editor.vue" })
// Read Vue component from apps/designer
...
// Update composition API setup

Bash({ command: "pnpm test:editor" })
// Run editor package tests
```
...
USER_NAME, I've refactored the Editor component to composition API and all tests are passing.

```bash
uv run .claude/hooks/utils/tts/elevenlabs_tts.py "USER_NAME, I've refactored the Editor component to composition API and all tests are passing."
```
