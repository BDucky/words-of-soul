---
name: GenUI
description: Generate standalone HTML files with modern styling, save to .ai/html-output-styles/ and open in browser
---
...
4. Save the HTML file to `.ai/html-output-styles/` with a descriptive name and `.html` extension (see `## File Output Convention` below)
...
<body>
    <article>
        ...
        <footer>
            <p>Apps: designer | enduser | auth | demo</p>
            <p>Packages: core | editor | extensions | plugins</p>
        </footer>
    </article>
</body>
</html>
...
1. Save to `.ai/html-output-styles/` directory with descriptive names
...
3. Save to `.ai/html-output-styles/` directory
...
- The last piece of your response should be two things.
  ...
  - A path to the generated HTML file, e.g. `.ai/html-output-styles/cc_genui_<concise description>_YYYYMMDD_HHMMSS.html`.